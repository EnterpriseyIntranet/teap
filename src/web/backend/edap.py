...
class LdapUserMixin:
    def add_user(self, uid, name, surname, password, mail, picture_bytes=b"", mail_aliases=None):
        if not mail_aliases:
            mail_aliases = []
        if self.subobject_exists_at("ou=active,ou=people", "organizationalUnit") == 0:
            raise ConstraintError(f"The people group '{self.ACTIVE_PEOPLE_GROUP}' doesn't exist.")
        ...
        self.add_s(f"uid={uid},{self.ACTIVE_PEOPLE_GROUP}", modlist)
    ...
    def get_users(self, search=None):
        ...
        Returns:
        """
        return self.get_subobjects('ou=active,ou=people', search, obj_class='inetOrgPerson')
    ...
    def modify_user(self, uid, modify_dict):
        fqdn = f"uid={uid},{self.ACTIVE_PEOPLE_GROUP}"
        transform = dict(
    ...
    def user_of_uid_exists(self, uid):
        if self.subobject_exists_at("ou=active,ou=people", "organizationalUnit") == 0:
            raise ConstraintError(f"The people group '{self.ACTIVE_PEOPLE_GROUP}' doesn't exist.")
        found = self.search_s(f"{self.ACTIVE_PEOPLE_GROUP}", ldap.SCOPE_ONELEVEL, f"(uid={uid})")
        return len(found)
    ...
    def delete_user(self, uid):
        return self.delete_object(f"uid={uid},{self.ACTIVE_PEOPLE_GROUP}")


class LdapPostfixUserMixin(LdapUserMixin):
    MAIL_GID = 5000
    MAIL_UID = 5000
    HOME_FORMAT_STR = "/var/mail/{mail_domain}/{uid}"

    def _get_postfix_dict(self, uid, mail_subdomain=None):
        mail_domain = self.domain
        if mail_subdomain:
            mail_domain = ".".join((mail_subdomain, self.domain))
...
class LdapDdeaMixin(LdapPostfixUserMixin):
    """
...
class LdapFranchiseMixin(object):
    ...
    def label_franchise(self, code):
        ...
        country_name = COUNTRIES_CODES.get(code, None)
        if not country_name:
...
class Edap(LdapObjectsMixin, LdapGroupMixin, OrganizationalUnitMixin,
    ...
    def __init__(self, hostname, admin_cn, password, domain=None):
        ...
        self.ACTIVE_PEOPLE_GROUP = f"ou=active,{self.PEOPLE_GROUP}"
